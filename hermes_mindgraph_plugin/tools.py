...
def retrieve_session_context() -> Optional[str]:
    ...
    header = (
        ...
        "## When to Write (do this naturally as conversation flows)\n"
        "Use the RIGHT tool for each type of knowledge:\n\n"
        "**Reality layer** (entities, observations, facts):\n"
        "- **mindgraph_capture**(capture_type='entity') — people, orgs, concepts, places discussed. Deduplicates.\n"
        "- **mindgraph_capture**(capture_type='observation') — factual observations about the world.\n\n"
        "**Epistemic layer** (claims, evidence, questions, hypotheses, concepts):\n"
        "- **mindgraph_argue** — structured claims with evidence + confidence. Core epistemic tool.\n"
        "- **mindgraph_inquire**(inquiry_type='question') — open questions worth tracking.\n"
        "- **mindgraph_inquire**(inquiry_type='hypothesis') — testable conjectures.\n"
        "- **mindgraph_inquire**(inquiry_type='anomaly') — things that don't fit existing models.\n"
        "- **mindgraph_capture**(capture_type='concept') — abstract concepts, theories, frameworks.\n\n"
        "**Intent layer** (goals, decisions, projects):\n"
        "- **mindgraph_commit** — goals, projects, milestones. User's own intent only.\n"
        "- **mindgraph_decide** — open/add_option/resolve decisions. Tracks deliberation.\n\n"
        "**Action layer** (risks, affordances):\n"
        "- **mindgraph_action**(action='assess_risk') — risks, threats, failure modes.\n"
        "- **mindgraph_action**(action='add_affordance') — capabilities, leverage points, opportunities.\n\n"
        "**Agent layer** (plans, tasks, execution, governance):\n"
        "- **mindgraph_plan**(action='create_task/create_plan') — track work to be done.\n"
        "- **mindgraph_plan**(action='start/complete/fail') — execution lifecycle.\n"
        "- **mindgraph_plan**(action='create_policy') — governance rules.\n\n"
        "**Memory layer** (sessions, journals):\n"
        "- **mindgraph_journal** — preferences, reflections, insights. Low-friction capture.\n"
        "- **mindgraph_ingest** — long-form content (articles, papers, documents).\n\n"
        "## When to Read\n"
        "- Basic topic-relevant context is already injected automatically each turn (proactive retrieval).\n"
        "- Use **mindgraph_retrieve** for deeper queries: targeted searches, checking specific goals/decisions, "
        "weak claims, contradictions, or when you need more context than what was auto-injected.\n\n"
        "## Judgment Calls\n"
        "- Don't journal every trivial exchange — focus on things that build the user's knowledge graph.\n"
        "- During coding/execution sessions, writing is less common. During planning, research, or "
        "reflective conversations, write more.\n"
        "- Post-session extraction also runs automatically, so you don't need to exhaustively capture "
        "everything — focus on high-signal items the user would want recalled.\n"
        "- Goals, projects, milestones = user's own intent only. Never commit content-derived goals."
    )
